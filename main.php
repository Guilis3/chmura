...
if($_COOKIE['login'] == 'zalogowany')
{
    ...
    $result1 = mysqli_query($link, "SELECT * FROM `logi` WHERE bledy >0 AND usrnm = '$user' ORDER BY data LIMIT 1 "); // pobranie z BD wiersza, w którym login=login z formularza
    ...
    if($rekord1['bledy']==1)
    {
    echo 'ostatnia nieudana proba logowania: '; echo $rekord1['data'];
    }
    echo '<br/>';
...
<form action="newdir.php" method="POST"  ENCTYPE="multipart/form-data">
<h>Tworzenie nowego katalogu</h></br>
<input type="input" name="dir"/></br>
<input type="submit" value="Stworz"/></br>
</form>
...
<link rel="stylesheet" type="text/css" href="http://img.vinastar.net/dlf/styles.css" />
<?php
if($showthumbnails) {
?>
...
<!--
function o(n, i) {
	document.images['thumb'+n].src = 'http://img.vinastar.net/dlf/i.php?f='+i<?php if($memorylimit!==false) echo "+'&ml=".$memorylimit."'"; ?>;

}

function f(n) {
	document.images['thumb'+n].src = 'http://img.vinastar.net/dlf/trans.gif';
}
//-->
</script>
<?php
...
<body>
<div id="container">
  <h1>Katalog: <?php echo str_replace('\\', '', dirname(strip_tags($_SERVER['PHP_SELF']))).'/'.$leadon;?></h1>
  <div id="breadcrumbs"> <a href="<?php echo strip_tags($_SERVER['PHP_SELF']);?>">Strona Glowna</a>
  <?php
  ...
  <div id="listingcontainer">
	...
	<div id="headerfile"><a href="<?php echo $baseurl . $fileurl;?>">Nazwa</a></div>
	<div id="headersize"><a href="<?php echo $baseurl . $sizeurl;?>">Rozmiar</a></div>
	<div id="headermodified"><a href="<?php echo $baseurl . $dateurl;?>">Ostatnia Modyfikacja</a></div>
	</div>
    <div id="listing">
	<?php
	$class = 'b';
	if($dirok) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode($dotdotdir);?>" class="<?php echo $class;?>"><img src="http://img.vinastar.net/dlf/dirup.png" alt="Folder" /><strong>..</strong> <em>&nbsp;</em>&nbsp;</a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
	?>
	<div><a href="<?php echo strip_tags($_SERVER['PHP_SELF']).'?dir='.urlencode(str_replace($startdir,'',$leadon).$dirs[$i]);?>" class="<?php echo $class;?>"><img src="http://img.vinastar.net/dlf/folder.png" alt="<?php echo $dirs[$i];?>" /><strong><?php echo $dirs[$i];?></strong> <em>-</em> <?php echo date ("d/m/y - H:i:s", filemtime($includeurl.$leadon.$dirs[$i]));?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
	...
	for($i=0;$i<$arsize;$i++) {
		...
		if($showthumbnails && in_array($ext, $supportedimages)) {
			$thumb = '<span><img src="http://img.vinastar.net/dlf/trans.gif" alt="'.$files[$i].'" name="thumb'.$i.'" /></span>';
	...
	?>
	<div><a href="<?php echo $fileurl;?>" class="<?php echo $class;?>"<?php echo $thumb2;?>><img src="http://img.vinastar.net/dlf/<?php echo $icon;?>" alt="<?php echo $files[$i];?>" /><strong><?php echo $filename;?></strong> <em><?php echo round(filesize($includeurl.$leadon.$files[$i])/1024);?> Kb</em> <?php echo date ("d/m/y - H:i:s", filemtime($includeurl.$leadon.$files[$i]));?><?php echo $thumb;?></a></div>
	<?php
		if($class=='b') $class='w';
		else $class = 'b';
	}
